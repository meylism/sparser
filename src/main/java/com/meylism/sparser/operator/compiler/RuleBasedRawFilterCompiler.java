...
import com.meylism.sparser.operator.utf.UTF8ExactMatchFilter;
...
//        rawFilters.addAll(valueTokens.stream().map(UTF8KeyValueMatchFilter::new).collect(Collectors.toList()));
  ...
  private Map<FileFormat, Map<PredicateSupport, List<RawFilterSupport>>> initMappingOfSupport() {
    ...
    jsonPredicateSupport.put(PredicateSupport.EXACT_MATCH,
        Arrays.asList(RawFilterSupport.UTF_SUBSTRING_SEARCH));
    jsonPredicateSupport.put(PredicateSupport.KEY_VALUE_MATCH,
        Arrays.asList(RawFilterSupport.UTF_KEY_VALUE_SEARCH));
