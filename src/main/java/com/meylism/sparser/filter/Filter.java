...
//  private OptimizationHandler optimizationHandler;
   ...
   * @param predicate
   */
  public Filter(Configuration configuration, Predicate predicate, List<Transformer> transformers, List<Optimizer> optimizers) {
  ...
  private Boolean applyFilter(Object record) {
    ...
    for (Set<FilterOperator> conjunction : cascade.getCascade()) {
      ...
      for (FilterOperator operator : conjunction) {
        if (operator.evaluate(record)) passesInConjunction++;
      }
      if (passesInConjunction == conjunction.size()) return false;
    }
    return true;
  }
